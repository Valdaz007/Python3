...
def compare_Rand(rand_input):
	...
	while counter > 0: 
		...
		if rand == rand_input:
			...
			print("\nRandom Number = {}. Your Number of Choices are Up. Try Again!".format(rand_input))
			break
		elif rand < rand_input:
			print("Input is Less Than The Random Number! Number of Choices Left {}".format(counter))
			continue
		elif rand > rand_input:
			print("Input is Greater Than The Random Number! Number of Choices Left {}".format(counter))
			continue
		#End If Statements
	#End While Loop
